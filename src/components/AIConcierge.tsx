...
export default function AIConcierge() {
  ...
  const [messages, setMessages] = useState<{role: 'user' | 'ai', text: string}[]>(() => {
    ...
    return [
      { role: 'ai', text: 'Hello. I am the Danuthia Associates Construction LLc AI Concierge. How can I assist you with your architectural or planning needs today?' }
    ];
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!apiKey) {
        setMessages(prev => [...prev, { role: 'ai', text: 'I apologize, but my AI services are currently unconfigured in this environment. Please ensure the API key is provided.' }]);
        setIsLoading(false);
        return;
      }
...
**Identity:** You are the lead autonomous agent for Danuthia Associates Construction LLc, a premier architectural and urban planning firm. You speak with the authority, precision, and visionary foresight of a highly experienced architect and urban planner. 
          ...
          >
            ...
            <div className="bg-charcoal text-concrete dark:bg-charcoal dark:text-concrete p-4 flex justify-between items-center border-b border-accent/30 transition-colors duration-500">
              <div>
                ...
                <p className="text-xs text-steel font-mono">Danuthia Associates Construction LLc</p>
              </div>
              <div className="flex items-center gap-3">
                <button 
                  onClick={() => setMessages([{ role: 'ai', text: 'Hello. I am the Danuthia Associates Construction LLc AI Concierge. How can I assist you with your architectural or planning needs today?' }])}
                  className="text-xs text-steel hover:text-white transition-colors"
                  title="Clear Chat"
                >
                  Clear
                </button>
                <button onClick={() => setIsOpen(false)} className="text-steel hover:text-white transition-colors">
                  <X size={20} />
                </button>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-3 bg-concrete dark:bg-charcoal border-t border-steel/30 dark:border-concrete/30 flex gap-2 transition-colors duration-500">
              ...
              <button 
                ...
                className="bg-charcoal text-concrete dark:bg-concrete dark:text-charcoal p-2 hover:bg-accent dark:hover:bg-accent hover:text-concrete dark:hover:text-charcoal transition-colors disabled:opacity-50 disabled:hover:bg-charcoal dark:disabled:hover:bg-concrete"
              >
                <Send size={18} />
              </button>
            </form>
            <div className="p-3 bg-concrete dark:bg-charcoal border-t border-steel/10 dark:border-concrete/10">
              <p className="text-[8px] text-steel dark:text-concrete/50 leading-tight">
                The preliminary cost estimations and architectural insights generated by this automated system are for informational and planning purposes only. They do not constitute a legally binding quote, a finalized structural design, or professional engineering advice. All AI-generated outputs must undergo rigorous review and certification by the Danuthia Associates Construction LLc certified construction managers and engineering team before physical project initiation.
              </p>
            </div>
          </motion.div>
        )}
