...
function processFile(path) {
  ...
  const regex = /<([a-zA-Z0-9_]+)([^>]*?)(>([^<]*?)\{\s*resources\.([a-zA-Z0-9_]+).*?\}.*?<\/[a-zA-Z0-9_]+>)/g;
  content = content.replace(regex, (match, tag, attr, rest, before, key) => {
    if (attr.includes('data-cms-key')) return match;
    return `<${tag} data-cms-key="${key}"${attr}${rest}`;
  });

  const regex2 = /<([a-zA-Z0-9_]+)([^>]*?)(>([^<]*?)\{\s*resources\['([a-zA-Z0-9_]+)'\].*?\}.*?<\/[a-zA-Z0-9_]+>)/g;
  content = content.replace(regex2, (match, tag, attr, rest, before, key) => {
    if (attr.includes('data-cms-key')) return match;
    return `<${tag} data-cms-key="${key}"${attr}${rest}`;
  });
...
['src/pages/Home.tsx', 'src/pages/About.tsx', 'src/pages/Services.tsx', 'src/pages/Portfolio.tsx', 'src/components/Footer.tsx', 'src/components/Header.tsx'].forEach(file => {
